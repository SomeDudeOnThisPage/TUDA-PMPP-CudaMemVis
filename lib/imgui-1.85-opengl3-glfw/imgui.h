...
namespace ImGui
{
    ...
    //  - BeginPopupModal(): block every interactions behind the window, cannot be closed by user, add a dimming background, has a m_title bar.
    ...
    // - Drag from window m_title bar or their tab to dock/undock. Hold SHIFT to disable docking/undocking.
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_UnsavedDocument        = 1 << 20,  // Display a dot next to the m_title. When used in a tab/docking context, tab is selected when clicking the X + closure is not assumed (will wait for user to stop submitting the tab). Otherwise closure is assumed when pressing the X, so if you keep submitting the tab may reappear at end of tab bar.
...
enum ImGuiTabItemFlags_
{
    ...
    ImGuiTabItemFlags_UnsavedDocument               = 1 << 0,   // Display a dot next to the m_title + tab is selected when clicking the X + closure is not assumed (will wait for user to stop submitting the tab). Otherwise closure is assumed when pressing the X, so if you keep submitting the tab may reappear at end of tab bar.
...
struct ImGuiIO
{
    ...
    bool        ConfigWindowsMoveFromTitleBarOnly; // = false       // Enable allowing to move windows only when clicking on their m_title bar. Does not apply to windows without a m_title bar.
...
enum ImGuiViewportFlags_
{
    ...
    ImGuiViewportFlags_NoDecoration             = 1 << 3,   // Platform Window: Disable platform decorations: m_title bar, borders, etc. (generally set all windows, but if ImGuiConfigFlags_ViewportsDecoration is set we only set this on popups/tooltips)
...
//   You may decide to setup the platform_io's *RenderWindow and *swap_buffers pointers and call your functions through those pointers,
...
struct ImGuiPlatformIO
{
    ...
    void    (*Platform_SetWindowSize)(ImGuiViewport* vp, ImVec2 size);      // . . U . .  // Set platform window client area size (ignoring OS decorations such as OS m_title bar etc.)
    ...
    void    (*Platform_SetWindowTitle)(ImGuiViewport* vp, const char* str); // . . U . .  // Set platform window m_title (given an UTF-8 string)
    ...
    void    (*Platform_SwapBuffers)(ImGuiViewport* vp, void* render_arg);   // . . . R .  // (Optional) Call Present/swap_buffers (platform side! This is often unused!). 'render_arg' is the value passed to RenderPlatformWindowsDefault().
    ...
    void    (*Renderer_SwapBuffers)(ImGuiViewport* vp, void* render_arg);   // . . . R .  // (Optional) Call Present/swap_buffers. 'render_arg' is the value passed to RenderPlatformWindowsDefault().
